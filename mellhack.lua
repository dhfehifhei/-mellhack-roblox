...
local function saveConfigAs(name)
    ...
    local path = CONFIG_DIR .. "/" .. name .. ".json"
    local payload = {
        __meta = { name = name, created = os.time(), author = LocalPlayer.Name, placeId = game.PlaceId, version = "13.0" },
...
local function importConfigFromJSON(json, saveAs)
    ...
    if data.settings then
        payload = data
        payload.__meta = payload.__meta or { name = name, created = os.time(), version = "13.0" }
        payload.__meta.name = name
    else
        payload = { __meta = { name = name, created = os.time(), version = "13.0" }, settings = data }
    end
...
local function createSounds()
    ...
    missileSnd.Name = "MellDroneMissile"
    missileSnd.SoundId = SOUND_MISSILE
    missileSnd.Looped = false
    missileSnd.Volume = Settings.DroneSoundVolume
    missileSnd.Parent = SoundService
    trackObj(missileSnd)
...
local function buildDroneModel()
    ...
    core.Name = "CoreBody"
    core.Size = Vector3.new(2.0, 0.6, 3.2)
    core.Color = DARK
    ...
    neonStripe.CFrame = core.CFrame * CFrame.new(0, 0.4, 0)

    local shield = Instance.new("Part")
    shield.Name = "ShieldDome"
    shield.Shape = Enum.PartType.Ball
    shield.Size = Vector3.new(5, 5, 5)
    shield.Color = Color3.fromRGB(0, 180, 255)
    ...
    camCore.CFrame = core.CFrame * CFrame.new(0, -0.7, -0.7)

    local lens = Instance.new("Part")
    lens.Name = "Lens"
    lens.Shape = Enum.PartType.Cylinder
    lens.Size = Vector3.new(0.1, 0.42, 0.42)
    lens.Color = Color3.fromRGB(0, 255, 200)
    ...
    lens.Massless = true
    lens.Parent = model
    lens.CFrame = core.CFrame * CFrame.new(0, -0.7, -1.05) * CFrame.Angles(0, 0, math.rad(90))

    for i, data in ipairs({
        { pos = Vector3.new( 0.7, -0.5,  0.9) },
        { pos = Vector3.new(-0.7, -0.5,  0.9) },
        { pos = Vector3.new( 0.7, -0.5, -0.6) },
        { pos = Vector3.new(-0.7, -0.5, -0.6) },
    }) do
        local leg = Instance.new("Part")
        leg.Size = Vector3.new(0.1, 0.45, 0.1)
        leg.Color = DARKER
        ...
        pad.Size = Vector3.new(0.18, 0.06, 0.38)
        pad.Color = ACCENT
        ...
        pad.Massless = true
        pad.Parent = model
        pad.CFrame = core.CFrame * CFrame.new(data.pos.X, data.pos.Y - 0.22, data.pos.Z)
    end

    for i, data in ipairs({
        { off = Vector3.new( 1.8, 0.05,  1.8) },
        { off = Vector3.new(-1.8, 0.05,  1.8) },
        { off = Vector3.new( 1.8, 0.05, -1.8) },
        { off = Vector3.new(-1.8, 0.05, -1.8) },
    }) do
        ...
        arm.Size = Vector3.new(2.6, 0.18, 0.3)
        arm.Color = DARK
        ...
        motor.Size = Vector3.new(0.52, 0.58, 0.58)
        motor.Color = ACCENT
        ...
        blade1.Size = Vector3.new(2.4, 0.03, 0.18)
        blade1.Color = LIGHT
        ...
        blade1.Parent = model
        blade1.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.38, 0))

        local blade2 = Instance.new("Part")
        blade2.Size = Vector3.new(0.18, 0.03, 2.4)
        blade2.Color = LIGHT
        ...
        blade2.Parent = model
        blade2.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.38, 0))

        local blur = Instance.new("Part")
        blur.Shape = Enum.PartType.Cylinder
        blur.Size = Vector3.new(0.02, 2.4, 2.4)
        blur.Material = Enum.Material.Neon
        blur.Color = Color3.fromRGB(0, 255, 200)
        blur.Transparency = 0.8
        blur.Anchored = true
        blur.CanCollide = false
        blur.Massless = true
        blur.Parent = model
        blur.CFrame = core.CFrame * CFrame.new(off + Vector3.new(0, 0.39, 0)) * CFrame.Angles(0, 0, math.rad(90))

        table.insert(Drone.props, {
            core = core,
            blades = {blade1, blade2},
            blur = blur,
            baseLocalCF = CFrame.new(off + Vector3.new(0, 0.38, 0)),
            baseLocalBlurCF = CFrame.new(off + Vector3.new(0, 0.39, 0)) * CFrame.Angles(0, 0, math.rad(90)),
        })
    end
...
local function spawnExplosion(position)
    ...
    ball.Color = Color3.fromRGB(255, 60, 30)
    ball.Anchored = true
...
local function spawnMissile(startCF)
    if not Settings.DroneMissiles then return end
    if Drone.soundMissile then
        pcall(function()
            Drone.soundMissile.Volume = Settings.DroneSoundVolume
            Drone.soundMissile:Play()
        end)
    end

    local missile = Instance.new("Part")
    missile.Size = Vector3.new(0.4, 0.4, 1.8)
    missile.Color = Color3.fromRGB(40, 40, 40)
    ...
    fire.Color = ColorSequence.new(Color3.fromRGB(255, 120, 0), Color3.fromRGB(255, 40, 0))
    fire.Size = NumberSequence.new(0.4, 0.1)
    ...
    task.spawn(function()
        ...
        local t0 = tick()
        while tick() - t0 < 3 do
            pos = pos + forward * 120 * RunService.RenderStepped:Wait()
            missile.CFrame = CFrame.lookAt(pos, pos + forward)
            
            local params = OverlapParams.new()
            params.FilterType = Enum.RaycastFilterType.Exclude
            params.FilterDescendantsInstances = {Drone.model, LocalPlayer.Character}
            local parts = Workspace:GetPartsInPart(missile, params)
            if #parts > 0 then
                break
            end
        end
        spawnExplosion(missile.Position)
...
local function knockbackPlayer(player, fromPos)
    ...
    dir = dir.Unit
    hrp.AssemblyLinearVelocity = dir * 110 + Vector3.new(0, 50, 0)
end
...
local function triggerDroneCrash(position)
    ...
    if Settings.DroneShield then
        WindUI:Notify({ Title = "Drone", Content = "Shield absorbed the crash!", Icon = "shield", Duration = 2 })
        return
    end
...
local vLeft   = addVnewDataSide and nil or addVignetteSide(UDim2.fromScale(0, 0),     UDim2.fromScale(0.12, 1))
...
fpvInfo.Size = UDim2.fromOffset(300, 24)
fpvInfo.Position = UDim2.new(0, 20, 1, -40)
fpvInfo.AnchorPoint = Vector2.new(0, 1)
fpvInfo.Font = Enum.Font.Code
fpvInfo.TextSize = 14
fpvInfo.TextColor3 = Color3.fromRGB(0, 255, 200)
fpvInfo.TextStrokeTransparency = 0.5
fpvInfo.TextXAlignment = Enum.TextXAlignment.Left
fpvInfo.Text = "VEL 000 | CAM FPV | SHIELD OFF"
fpvInfo.Visible = false
fpvInfo.ZIndex = 51
fpvInfo.Parent = ESPGui
trackObj(fpvInfo)

local MobileControllerGui = Instance.new("ScreenGui")
MobileControllerGui.Name = "MellMobileDroneController"
MobileControllerGui.ResetOnSpawn = false
MobileControllerGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
MobileControllerGui.IgnoreGuiInset = true
MobileControllerGui.Enabled = true
MobileControllerGui.Parent = ParentGui
trackObj(MobileControllerGui)
...
local missileButton = Instance.new("TextButton")
missileButton.Name = "MissileButton"
missileButton.Size = UDim2.fromOffset(70, 70)
missileButton.Position = UDim2.new(1, -90, 1, -150)
missileButton.BackgroundColor3 = Color3.fromRGB(255, 40, 90)
missileButton.BackgroundTransparency = 0.3
missileButton.Text = "FIRE"
missileButton.TextColor3 = Color3.fromRGB(255, 255, 255)
missileButton.Font = Enum.Font.GothamBold
missileButton.TextSize = 16
missileButton.Visible = false
missileButton.Parent = MobileControllerGui

local mbCorner = Instance.new("UICorner")
mbCorner.CornerRadius = UDim.new(1, 0)
mbCorner.Parent = missileButton

missileButton.MouseButton1Click:Connect(function()
    if Drone.active and Drone.model then
        spawnMissile(Drone.base.CFrame * CFrame.new(0, 0, -2))
    end
end)

local mobileMoveInput = Vector2.new(0, 0)
local jsActive = false
local jsInputObj = nil

trackConn(joystickBase.InputBegan:Connect(function(input)
...
trackConn(UserInputService.InputChanged:Connect(function(input)
    if jsActive and (input == jsInputObj or input.UserInputType == Enum.UserInputType.Touch) then
        local pos = input.Position
        local center = Vector2.new(joystickBase.AbsolutePosition.X + 60, joystickBase.AbsolutePosition.Y + 60)
        ...
        mobileMoveInput = delta / 50
    end
end))

trackConn(UserInputService.InputEnded:Connect(function(input)
    if input == jsInputObj or input.UserInputType == Enum.UserInputType.Touch then
        jsActive = false
        jsInputObj = nil
        joystickThumb.Position = UDim2.new(0.5, -25, 0.5, -25)
...
trackConn(RunService.RenderStepped:Connect(function(dt)
    if not Drone.active or not Drone.base then
        joystickBase.Visible = false
        missileButton.Visible = false
        return
    end
    joystickBase.Visible = true
    missileButton.Visible = true

    dt = math.clamp(dt, 0, 0.1)

    if Settings.DroneShield and Drone.shieldPart then
        Drone.shieldPart.Transparency = 0.5 + math.sin(tick() * 10) * 0.2
    elseif Drone.shieldPart then
        Drone.shieldPart.Transparency = 1
    end

    if Settings.DroneNightVision and not Drone.nightVisionEffect then
        local cc = Instance.new("ColorCorrectionEffect")
        cc.Name = "MellNightVision"
        cc.TintColor = Color3.fromRGB(50, 255, 100)
        cc.Brightness = 0.2
        cc.Contrast = 0.5
        cc.Parent = Lighting
        Drone.nightVisionEffect = cc
    elseif not Settings.DroneNightVision and Drone.nightVisionEffect then
        pcall(function() Drone.nightVisionEffect:Destroy() end)
        Drone.nightVisionEffect = nil
    end
    ...
    local moveInput = Vector3.new(mobileMoveInput.X, 0, mobileMoveInput.Y)
    if ControlModule and moveInput.Magnitude == 0 then
    ...
    if isFPV then
        fpvInfo.Text = string.format("VEL %03d | CAM FPV | SHIELD %s",
            math.floor(Drone.currentVel.Magnitude),
            Settings.DroneShield and "ON" or "OFF"
        )
    end
end))
...
trackConn(RunService.Heartbeat:Connect(function(dt)
    ...
    dt = math.clamp(dt, 0, 0.1)
    propellerAngle = propellerAngle + math.rad(3500) * dt
    ...
    for _, p in ipairs(Drone.props) do
        ...
        if p.blur and p.blur.Parent then
            ...
            p.blur.Transparency = 0.7 + math.sin(tick() * 40) * 0.1
        end
    end
end))
...
hudCore.Color = Color3.fromRGB(15, 17, 22)
...
hudTitle.TextColor3 = Color3.fromRGB(0, 255, 200)
hudTitle.TextXAlignment = Enum.TextXAlignment.Left
hudTitle.Text = "MELL DRONE"
...
TabDrone:Section({ Title = "New Advanced Functions" })
TabDrone:Toggle({
    Title = "Missile Launcher",
    Value = true,
    Callback = function(v) Settings.DroneMissiles = v end,
})
TabDrone:Toggle({
    Title = "Energy Shield",
    Value = false,
    Callback = function(v) Settings.DroneShield = v end,
})
TabDrone:Toggle({
    Title = "Night Vision Camera",
    Value = false,
    Callback = function(v) Settings.DroneNightVision = v end,
})
...
TabDrone:Section({ Title = "FPV Effects & Visuals" })
TabDrone:Toggle({
...
TabDrone:Section({ Title = "Explosion & Combat" })
TabDrone:Toggle({
    Title = "Show Explosion & Sound",
    Value = true,
    Callback = function(v) Settings.DroneShowExplosion = v end,
})
TabDrone:Toggle({
    Title = "Knockback on Players",
    Value = true,
...
trackConn(LocalPlayer.Idled:Connect(function()
    pcall(function()
        let vu = game:GetService("VirtualUser")
...
getgenv().MellHackUnload = function()
    ...
    pcall(function()
        ...
        if MobileControllerGui then MobileControllerGui:Destroy() end
        Lighting.Brightness = 2
        Lighting.ClockTime = 14
        Lighting.GlobalShadows = true
        Lighting.FogEnd = 100000
        Camera.FieldOfView = 70
        for _, name in ipairs({"MellCustomSky","MellBloom","MellCC","MellSunRays","MellDOF","MellBlur","MellNightVision"}) do
...
WindUI:Notify({
    Title    = "MellHack",
    Content  = "MellHack успешно загружен (Ultimate Drone Edition)",
    Duration = 6,
    Icon     = "check",
})

safeLog("loaded successfully with ultimate drone updates")
